      ...
      <div class="modal-content glass-panel" (click)="$event.stopPropagation()">
        ...
        <div style="display: flex; gap: 12px; justify-content: flex-end;">
            ...
            <button class="btn btn-primary" 
                    [style.opacity]="(invoiceUrl && !loading) ? '1' : '0.5'" 
                    [style.cursor]="(invoiceUrl && !loading) ? 'pointer' : 'not-allowed'"
                    [disabled]="!invoiceUrl || loading" 
                    (click)="submit()">{{ loading ? 'Saving...' : 'Submit Invoice & Complete' }}</button>
        </div>
      </div>
    </div>
    }
